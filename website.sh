...
if [ "$server_name" = "" ]; then
  ...
  echo "Ban nhap sai, he thong dung mydomain lam ten mien chinh"
fi

printf "\nNhap vao port admin (9889) roi an [ENTER]: " 
read admin_port
...
yum -y remove mysql* php* httpd* sendmail* postfix*
yum clean all
...
if [ "$php_version" = "7.1" ]; then
  ...
  yum -y install nginx php-fpm php-common php-gd php-mysqlnd php-pdo php-xml php-mbstring php-mcrypt php-curl php-opcache php-cli php-pecl-zip php-pecl-redis4
...
# Install Others
yum -y install exim unzip zip openssl
...
cat > "/etc/php.d/00-vps-custom.ini" <<END
...
printf "\nSetup config Nginx 1-Yes Others-No [ENTER]: " 
read nginx_yes
if [ "$nginx_yes" == "1" ]; then
...
root_password=`date |md5sum |cut -c '16-30'`
sleep 1
# Random password for MySQL admin account
admin_password=`date |md5sum |cut -c '16-30'`
...
END
chmod 600 /root/.scripts.conf

clear
cat > "/root/lamp-script.txt" <<END
=========================================================================
                           MANAGE VPS INFORMATION                        
=========================================================================
Menu
Domain chinh: http://$server_name/ hoac http://$server_ip/
Script Admin:  http://$server_name:$admin_port/ hoac http://$server_ip:$admin_port/
...
chmod 600 /root/lamp-script.txt
...
printf "=========================================================================\n"
printf "Thong tin quan ly duoc luu tai: /root/lamp-script.txt \n"
printf "=========================================================================\n"
printf "=========================================================================\n"
exit
